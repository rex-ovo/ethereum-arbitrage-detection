...
from utils import connect_web3, setup_logging, erc20
from utils.throttler import BlockThrottle

l = logging.getLogger(__name__)


def _get_transfer_logs_with_backoff(
        w3: web3.Web3,
        start_block: int,
        end_block_inclusive: int,
    ) -> typing.List[web3.types.LogReceipt]:
    """
    Fetch ERC20 Transfer logs for the given block range.

    Some RPC providers (notably Infura) reject log queries that would return
    more than 10,000 results. In that case, recursively split the range until
    the request fits within provider limits.
    """
    try:
        f: web3._utils.filters.Filter = w3.eth.filter({
            'fromBlock': start_block,
            'toBlock': end_block_inclusive,
        ...
        midpoint = (start_block + end_block_inclusive) // 2
        l.warning(
            'Transfer log query limit hit for %s-%s, splitting into %s-%s and %s-%s',
            f'{start_block:,}',
            f'{end_block_inclusive:,}',
            f'{start_block:,}',
            f'{midpoint:,}',
            f'{midpoint + 1:,}',
            f'{end_block_inclusive:,}',
        )
        left = _get_transfer_logs_with_backoff(w3, start_block, midpoint)
        right = _get_transfer_logs_with_backoff(w3, midpoint + 1, end_block_inclusive)
        return left + right
...
def process_reservation(
    ...
    while this_block_start < reservation_end_exclusive:
        ...
        logs = _get_transfer_logs_with_backoff(
            w3,
            this_block_start,
            this_end_block_inclusive,
        )
